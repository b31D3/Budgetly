...
export const calculateSemesterBreakdown = (
  ...
  // Upper bound: at most 2 periods per semester (1 academic + 1 summer between); break handles exact stopping
  const totalPeriodsToShow = numSemesters * 2;
  ...
  for (let periodIndex = 0; periodIndex < totalPeriodsToShow; periodIndex++) {
    ...
    // Stop once all academic semesters are shown — summers don't trail after the final semester
    if (academicSemesterCount >= numSemesters) {
      break;
    }
